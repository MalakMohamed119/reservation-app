...
import '../models/restaurant.dart' hide User; 
import '../models/user.dart' as user_model; 
// If Reservation is a separate file, uncomment the line below:
...
class RestaurantService extends ChangeNotifier {
  ...
  Future<void> makeReservation({
    required String restaurantId,
    required String tableId,
    required user_model.User customer, // 🌟 FIX: Require the User object directly
    required DateTime date,
    required String timeSlot,
    required int numberOfPeople,
    ...
    for (final restaurant in _restaurants) {
      ...
      for (final table in restaurant.tables) {
        ...
        final reservationsForUser = table.reservations.where(
          (reservation) => reservation.userId == userId && !reservation.isCancelled,
        );
